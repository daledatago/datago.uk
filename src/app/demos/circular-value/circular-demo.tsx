    ...
    <div className={styles.page}>
      ...
      <div className={styles.grid}>
        ...
        <section className={styles.panel} aria-live="polite">
          ...
          <h2>What changes?</h2>
          <div className={styles.metric}><span>Cost difference in this narrow model</span><strong className={netDifference >= 0 ? styles.positive : styles.negative}>{netDifference >= 0 ? "+" : ""}{money(netDifference)}</strong><small>{netDifference >= 0 ? "Lower modelled cost with reuse" : "Higher modelled cost with reuse"}</small></div>
          ...
          <p className={styles.analysis}>{refurbishment > materialCost * (1 + uplift / 100) ? "Refurbishment currently costs more than new material in this model. Test process efficiency or other sources of value before making the case." : `At these inputs, refurbishing costs less than buying new material. The result still depends on whether ${recovery}% recovery is achievable.`}</p>
        </section>
      </div>
      <div className={styles.grid}>
        <section className={styles.panel}>
          ...
          <p>With {recovery}% recovery, the model would need {annualRecovered.toLocaleString("en-GB")} fewer new units. A material price increase affects fewer purchases. This is a resilience hypothesis, not a measured supply-chain outcome.</p>
